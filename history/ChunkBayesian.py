...
class ChunkBayesian:
    ...
    def fit(self, tr_chunk, ts_chunk):
        ...
        for tr_idx, val_idx in skf.split(X):
            ...
            model = BayesianRidge(tol=self.model_config['tol'],
                      alpha_1=self.model_config['alpha_1'],
                      alpha_2=self.model_config['alpha_2'],
                      lambda_1=self.model_config['lambda_1'],
                      lambda_2=self.model_config['lambda_2'])
            model.fit(tr_X, tr_Y)
    ...
    def predict(self,X):
        ...
        return Y_pred
    def __str__(self):
        return "ChunkBayesian(kfold="+str(self.kf_k)+",seed="+str(self.seed)+')'
